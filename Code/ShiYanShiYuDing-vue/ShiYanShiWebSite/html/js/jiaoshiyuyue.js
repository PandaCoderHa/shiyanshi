...
$(function () {
    ...
    app.getBuKeYongShiJianByShiYanShi(sysid);
    setBuKeYongShiJan();

    $("#tijiao").click(tiJiaoYuYue);
    $("#kaishi").change(kaishiChange);
    $("#jieshu").change(setYiYuYueZuoWei);
    app.getYuYue(-1, sysid);
    setYiYuYueZuoWei();
    kaishiChange();
    if (app.loginuser.id.length == 8) {
        setChengYuan();
...
function setChengYuan(selected = false) {
    if (app.loginuser.students) {
        var studentsOptions = "";
        app.loginuser.students.forEach(function (s) {
            studentsOptions +=
                "<option " + (selected ? "selected" : "") + ' value="' + s.card_number + '">' + s.card_number + "-" + s.name + "</option>";
        });
...
function tiJiaoYuYue() {
    ...
    var bukeyong = app.BuKeYongShiJianS.find(b => b.zhouji == d.getDay());
    if (bukeyong) {
    ...
    if (app.loginuser.id.length == 8) {
        sc.find("selected").seatIds.forEach(function (selectedId, i) {
            ...
            var student = app.loginuser.students.find(s => s.card_number == $("#chengyuan").val()[i])
            console.log(student);
            var yuyue = {
                jiaoshibianhao: app.loginuser.id,
        ...
        var zuowei =
            zuoweiObjs[selectedId.split("_")[0]][selectedId.split("_")[1] - 1];
        var yuyue = {
            xueshengbianhao: app.loginuser.id,
            xueshengxingming: app.loginuser.name,
            banji: app.loginuser.classes,
            yuyuekaishiriqi: riqi + " " + $("#kaishi").val(),
            yuyuejieshuriqi: riqi + " " + $("#jieshu").val(),
            gengxinriqi: new Date().toString().substr(0, 24),
            xiangmu: $("#xiangmu").val(),
...
function setShiYanShiMing() {
    setTimeout(function () {
        if (app.ShiYanShiS.length > 0) {
            var s = app.ShiYanShiS.filter(s => s.zidongbianhao == sysid)[0];
            if (s) {
...
function setBuKeYongShiJan() {
    setTimeout(function () {
        if (app.BuKeYongShiJianS.length > 0) {
            //$("#shiyanshi").text(ShiYanShiS.find(s => s.zidongbianhao == getUrlParamater("sysid")).mingzi);
            var d = new Date(riqi);
            var zhoushu = getYearWeek(d);
            var bukeyong = app.BuKeYongShiJianS.find(b => b.kaishizhou <= zhoushu && zhoushu <= b.jieshuzhou && b.zhouji == d.getDay());
            if (bukeyong) {
...
function setYiYuYueZuoWei() {
    setTimeout(function () {
        if (app.YuYueS && app.YuYueS.length > 0) {
