...
$(function() {
    ...
    if (loginuser.id == "admin") {
        ...
        $(".m-s-menu ul").html('<li class="learn"><span class="fa fa-th-list"></span> 管理列表</li>');
        mulu.forEach(function(m) {
            $("#manager").append('<li><a href="' + m[1] + '">' + m[0] + '</a></li>');
            if (pathname.indexOf(m[1]) > 0) {
                $(".m-s-menu ul").append('<li class="crt"><a href="' + m[1] + '"><span class="glyphicon glyphicon-play"></span>' + m[0] + '</a></li>');

            } else {
                $(".m-s-menu ul").append('<li><a href="' + m[1] + '">' + m[0] + '</a></li>');
            }
        });
        $("#manager").append('<li><a id="logout" ui-sref="access.signin">退出登录</a></li>');
        ...
        // $(".m-s-menu ul").append((pathname.indexOf('jiaoshi-manager') > 0 ? classname : '<li>') + '<a href="jiaoshi-manager.html">实验室管理</a></li>');
        // $(".m-s-menu ul").append((pathname.indexOf('zuowei-manager') > 0 ? classname : '<li>') + '<a href="zuowei-manager.html">实验室座位管理</a></li>');
        // $(".m-s-menu ul").append((pathname.indexOf('jiaoshi-set') > 0 ? classname : '<li>') + '<a href="jiaoshi-set.html">实验室不可用时间设定</a></li>');
        // $(".m-s-menu ul").append((pathname.indexOf('report') > 0 ? classname : '<li>') + '<a href="report.html">报表</a></li>');
        // $("#manager li:lt(4)").each(function(i, li) {
        //     $(".m-s-menu ul").append(li.outerHTML);
        // });
        //$(".m-s-menu ul").html($("#manager").html());
        $("span:contains('个人中心')").parent().parent().hide();
    } else {
                ...
                if ($("#shiyanshiList tbody").length > 0) {
                    response.forEach(function(sys) {
                        $("#shiyanshiList tbody").append(
                            ...
                            '<td> <a href="#" data-id="' +
                            sys.zidongbianhao +
                            '" data-toggle="modal" data-target="#myModal">修改</a> | <a href="javascript:ShanChuan(' +
                            sys.zidongbianhao +
                            ')" >删除</a></td></tr>'
                        );
                    }, this);
                ...
                if (response) {
                    ...
                    response.forEach(function(b, i) {
                        $("#bukeyongTable").append(
                            ...
                            b.jieshuriqi +
                            "</td>" +
                            '<td><a href="＃" data-toggle="modal" data-target="#myModal" data-id="' +
                            b.zidongbianhao +
                            '">修改</a>｜<a href="javascript:shanchu(' +
                            b.zidongbianhao +
                            ')">删除</a></td></tr>'
                        );
                    }, this);
